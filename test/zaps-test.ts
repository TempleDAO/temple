...
import { SandalwoodToken } from "../typechain/SandalwoodToken";
import { SandalwoodToken__factory } from "../typechain/factories/SandalwoodToken__factory";
...
describe.only("Zaps Test", async () => {
  ...
  beforeEach(async () => {
    ...
    await sandalwoodToken.transfer(await verifier.getAddress(), toAtto(4));
    await sandalwoodToken.connect(verifier).increaseAllowance(openingCeremony.address, toAtto(4));
  ...
  it("Guests OpeningCeremony Zaps ETH", async () => {
    ...
    for (let i = 0; i < stakers.length; i++) {
      await sandalwoodToken.transfer(await stakers[i].getAddress(), toAtto(1));
      await sandalwoodToken.connect(stakers[i]).increaseAllowance(openingCeremony.address, toAtto(1));

...
/**
 * @title Epoch Payments contract. Epoch increase incrementally. Epoch allocations are set once for an epoch and subsequent updates
...
contract EpochPayments is IEpochPayments, PaymentBase, TempleElevatedAccess {
    ...
    function updateEpochPayments(
        ...
        if (block.timestamp >= epochStartTimes[currentEpochCache] + minEpochDuration) { revert EpochEnded(); }
        uint256 _length = recipients.length;
        if (_length != amounts.length) { revert AllocationsLengthMismatch(); }
        address recipient;
        for (uint256 i; i < _length; ++i) {

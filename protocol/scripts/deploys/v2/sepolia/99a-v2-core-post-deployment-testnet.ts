...
async function main() {
  ...
  {
    await mine(templeV2contracts.temple.addMinter(TEMPLE_V2_DEPLOYED.CORE.EXECUTOR_MSIG));
    await mine(templeV2contracts.temple.addMinter(await owner.getAddress()));
  }
  ...
  // Setup Temple => dTemple & base strategy
  {
    ...
    await mine(templeV2contracts.temple.addMinter(TEMPLE_V2_DEPLOYED.STRATEGIES.TEMPLE_BASE_STRATEGY.ADDRESS));

...
async function main() {
    ...
    // Setup Circuit Breaker
    {
        ...
        await mine(TEMPLE_V2_INSTANCES.circuitBreakerProxy.setIdentifierForCaller(
            TEMPLE_V2_ADDRESSES.TEMPLE_LINE_OF_CREDIT.ADDRESS,
            "EXTERNAL_USER"
        ));

        // Circuit breaker for Temple
        await mine(TEMPLE_V2_INSTANCES.circuitBreakerProxy.setCircuitBreaker(
            EXTERNAL_ALL_USERS,
        ...
        await setExplicitAccess(
            TEMPLE_V2_INSTANCES.tlcCircuitBreakerDai,
            TEMPLE_V2_ADDRESSES.CORE.CIRCUIT_BREAKER_PROXY,
            ["preCheck"],
            true
        );
        await setExplicitAccess(
            TEMPLE_V2_INSTANCES.tlcCircuitBreakerTemple,
            TEMPLE_V2_ADDRESSES.CORE.CIRCUIT_BREAKER_PROXY,
            ["preCheck"],
            true
        );
    ...
    {   
        ...
        await mine(TEMPLE_V2_INSTANCES.trv.addStrategy(
            TEMPLE_V2_ADDRESSES.STRATEGIES.TLC_STRATEGY.ADDRESS,
            0, // underperformingEquityThreeshold
            debtCeiling
        ));

        await mine(TEMPLE_V2_INSTANCES.tlc.setTlcStrategy(TEMPLE_V2_ADDRESSES.STRATEGIES.TLC_STRATEGY.ADDRESS));
    }

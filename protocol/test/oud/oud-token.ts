...
import { BigNumberish, Contract, Signer, TypedDataDomain, TypedDataField } from "ethers";
...
describe("Oud Token", async () => {
    ...
    async function recoverToken(token: Contract, amount: BigNumberish, signer: Signer) {
        const balBefore = await token.balanceOf(await signer.getAddress());
        await expect(token.recoverToken(token.address, await signer.getAddress(), amount))
    ...
    it("owner can recover tokens", async () => {
        ...
        await recoverToken(token, amount, owner);
    });

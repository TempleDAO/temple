...
export const WalletProvider = (props: PropsWithChildren<any>) => {
    ...
    if (walletAddress && signerState) {
      ...
      try {
        ...
        const sig = JSON.parse(
          Buffer.from(sandalWoodToken, 'base64').toString('ascii')
        );
        console.info(sig, sandalWoodToken);

        const verifyTransaction = await verifyQuestContract.verify(
          sig.v,
          sig.r,
          sig.s,
          {
  ...
  const increaseAllowanceForRitual = async (
    ...
    if (walletAddress && signerState) {
      ...
      const stableCoinAllowance = await stableCoinContract.allowance(
        walletAddress,
      ...
      if (stableCoinAllowance.lt(amount)) {
        try {
          ...
          const stableApproveTransaction = await stableCoinContract.approve(
            OPENING_CEREMONY_ADDRESS,
            ONE_MILLION
          );

...
describe("Temple Treasury management", async () => {
  ...
  describe("Treasury ops tests", async () => {
    beforeEach(async () => {
    ...
    it("Only owners can mint and allocate TEMPLE", async () => {
      ...
      await shouldThrow(TREASURY.connect(nonOwner).mintAndAllocateTemple(nonOwnerAddress, toAtto(500)), /caller is not the owner/);
      ...
      await shouldThrow(TREASURY.connect(nonOwner).unallocateAndBurnUnusedMintedTemple(nonOwnerAddress), /caller is not the owner/);
    ...
    it("Only owners can allocate STABLEC", async () => {
      ...
      await shouldThrow(TREASURY.connect(nonOwner).allocateTreasuryStablec(treasuryAllocation.address, toAtto(10)), /caller is not the owner/);
    ...
    it("Only owners can unsafe withdraw", async () => {
      ...
      await shouldThrow(TREASURY.connect(nonOwner).ejectTreasuryAllocation(treasuryAllocation.address), /caller is not the owner/);
    ...
    it("Only owners can add or remove pools", async () => {
      ...
      await shouldThrow(TREASURY.connect(nonOwner).upsertPool(nonOwnerAddress, 1), /caller is not the owner/);
      ...
      await shouldThrow(TREASURY.connect(nonOwner).upsertPool(nonOwnerAddress, 2), /caller is not the owner/);
      ...
      await shouldThrow(TREASURY.connect(nonOwner).removePool(0, nonOwnerAddress), /caller is not the owner/);

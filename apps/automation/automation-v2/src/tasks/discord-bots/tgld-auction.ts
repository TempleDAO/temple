...
export async function startAuctionSidebarBot(runner: TaskRunner) {
  return startDiscordSidebarBot({
    runner,
    botLabel: 'TGLD auction sidebar bot',
    tokenVariable: vars.tgld_auction_bot_token,
  });
}

export async function updateTgldAuctionBot({
  bot,
  getTgldAuctionState,
  logger,
}: {
  ...
  try {
    ...
    await applyDiscordSidebarState({
      bot,
      state: formatTgldAuctionSidebarState(stats),
      logger,
    });
  } catch (error) {
    logger.error(`Error updating TGLD auction bot`);
...
export function formatTgldAuctionSidebarState(
  stats: TGLDAuctionState,
  now: Date = new Date()
): SidebarState {
  switch (stats.kind) {
    case 'auction-ended': {
      ...
      return {
        nickname: formatTgldPrice(stats.data.lastEpoch.tgldPrice),
        activity: {
          type: 'custom',
          name: 'TGLD Auction',
          url: 'https://templedao.link/dapp/spice/earn/auctions',
          state: `Auction starts in ${startsIn}`,
        },
      };
    }
    case 'auction-in-progress': {
      ...
      return {
        nickname: formatTgldPrice(stats.data.currentAuction.tgldPrice),
        activity: {
          type: 'custom',
          name: 'TGLD Auction',
          url: 'https://templedao.link/dapp/spice/earn/auctions',
          state: `Auction ends in ${endsIn}`,
        },
      };
    }
  }
...
export async function getOnchainTgldAuctionState(
  ...
  const auctionDetails = async (epochId: bigint) => {
    const { totalBidTokenAmount, startTime, endTime, totalAuctionTokenAmount } =
  ...
  if (await auction.read.isCurrentEpochEnded()) {
    ...
    const nextAuctionStartTime = new Date(
      currentAuction.endsAt.getTime() +
        Number(config.auctionsTimeDiff + config.auctionStartCooldown) * 1000
    );
    return {
      kind: 'auction-ended',
      data: {

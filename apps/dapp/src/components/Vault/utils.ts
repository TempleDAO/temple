...
export const createVaultGroup = (
  subgraphVaultGroup: GraphVaultGroup
): VaultGroup => {
  ...
  const cyclesSinceStart = Math.floor(
    secondsSinceStart / periodDurationSeconds
  );

  vaultGroup.cycleStart = addSeconds(
    startDate,
    cyclesSinceStart * periodDurationSeconds
  );
  vaultGroup.cycleEnd = addSeconds(
    vaultGroup.cycleStart,
    periodDurationSeconds
  );
...
export const createVault = (subgraphVault: GraphVault): Partial<Vault> => {
  const startDate = createDateFromSeconds(
    subgraphVault.firstPeriodStartTimestamp
  );
  ...
  const enterExitWindowDurationSeconds = Number(
    subgraphVault.enterExitWindowDuration
  );
  ...
  const vaultUserBalance = userBalances.find(({ id }) =>
    id.startsWith(subgraphVault.id)
  );
  ...
  const vault: Partial<Vault> = {
    ...
    amountStaked: !!vaultUserBalance?.staked
      ? parseUnits(vaultUserBalance?.staked, 18)
      : BigNumber.from(0),
  };
...
export const getMarkers = (
  vaultGroup: Omit<VaultGroup, 'markers'>,
...
const calculateUnlockDate = (vault: Vault) => {
  ...
  const secondsUntillEndOfSycle =
    vault.periodDurationSeconds - secondsIntoThisCycle;
...
const calculateWindowEndDate = (vault: Vault) => {
  ...
  const secondsIntoWindow =
    secondsSinceStart % vault.enterExitWindowDurationSeconds;
  const secondsUntilEndOfWindow =
    vault.enterExitWindowDurationSeconds - secondsIntoWindow;
...
const calculateInZoneVaultInstance = (
  ...
  return (
    currentCycle * periodDurationSeconds +
      startSeconds +
      enterExitWindowDurationSeconds >
    nowSeconds
  );
...
export const formatTemple = (templeValue: Nullable<number | BigNumber>) => {
  ...
  const amount =
    typeof templeValue === 'number' ? templeValue : fromAtto(templeValue);

  return millify(amount, { precision: 4 });
};

export const getBigNumberFromString = (
  number: string,
...
export const formatJoiningFee = (
  stakeAmount: BigNumber,
  joiningFee: BigNumber
) => {

...
const EnclaveCard = styled.div<{ enclave: RelicEnclave, selected?: boolean }>`
  ...
  background-color: ${props => lighten(props.selected ? .2 : -.1, props.theme.palette.enclave[getEnclavePalette(props.enclave)])};
  background-blend-mode: multiply;
  background-size: cover;
  background-position: center;
  border: 2px solid;
  border-color: ${props => lighten(props.selected ? .2 : 0, props.theme.palette.enclave[getEnclavePalette(props.enclave)])};
  border-radius: 8px;
  transition: ease .25s all;
  transform: scale(${props => props.selected ? 1.1 : 1});
  z-index: ${props => props.selected ? 2 : 1};
  color: ${props => darken(props.selected ? .2 : .5, '#FFFFFF')};
  cursor: pointer;

  &:hover {
    background-color: ${props => lighten(props.selected ? .25 : .2, props.theme.palette.enclave[getEnclavePalette(props.enclave)])};
    border-color: ${props => lighten(props.selected ? .25 : .2, props.theme.palette.enclave[getEnclavePalette(props.enclave)])};
    color: ${props => darken(props.selected ? .1 : .3, '#FFFFFF')};
  }
...
function getEnclaveImg(enclave: RelicEnclave) {
  switch(enclave) {
    case RelicEnclave.Logic: return logicImg
    case RelicEnclave.Structure: return structureImg
    case RelicEnclave.Order: return orderImg
    case RelicEnclave.Mystery: return mysteryImg
    case RelicEnclave.Chaos: return chaosImg
  }
}

...
import openInNewTabImage from '../../public/images/open-in-new.svg';
...
const ETHERSCAN_DOMAIN = process.env.NODE_ENV === 'production' ? 'https://etherscan.io' : 'https://rinkeby.etherscan.io';
const Notification = ({ hash, title }: NotificationProps) => {
  ...
  const close = () => {
    ...
    containerRef?.current?.addEventListener('transitionend', () => closeNotification(hash), {
      once: true,
    });
  };
  ...
  return <NotificationStyled isOpen={isOpen} ref={containerRef}>
    <CloseIcon>
      <Image src={crossImage} alt={'Close notification'} width={24} height={24}
             onClick={close} objectPosition={'right'}/>
    </CloseIcon>
    <h5 title={title}>{title}</h5>
    <a href={`${ETHERSCAN_DOMAIN}/tx/${hash}`} target={'_blank'} rel="noreferrer">
      View Transaction

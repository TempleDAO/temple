...
export const Tabs = ({ tabs, onChange }: TabsProps) => {
  ...
  const getTabsContents = () => {
    ...
    const activeTabData: Tab | undefined = tabs.filter((tab => tab.label === activeTab))[0];
    if (activeTabData?.disabledMessage) {
      return <>
        <TabDisabledMessage>{activeTabData.disabledMessage}</TabDisabledMessage>
      </>;
    }
...
const TabStyled = styled(Button)<TabStyledProps>`
  ...
  `};
  transition: ${(props) => props.theme.transitions.color}, ${(props) => props.theme.transitions.backgroundColor};
  margin: 0;
